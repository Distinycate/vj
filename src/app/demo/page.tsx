...
function DemoOnboarding({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 sm:p-6 safe-bottom relative overflow-x-hidden"
    >
      ...
      {/* Demo badge */}
      <div className="absolute top-4 sm:top-6 left-4 sm:left-6 right-4 sm:right-6 flex flex-col min-[420px]:flex-row min-[420px]:justify-between min-[420px]:items-center gap-2">
        ...
        <button
          onClick={onStart}
          className="min-h-10 px-3 py-2 text-slate-400 hover:text-slate-200 bg-slate-900/60 hover:bg-slate-800/80 border border-slate-800 rounded-xl text-xs font-bold transition-colors"
        >
          ข้ามขั้นตอนแนะนำ →
        </button>
      </div>
...
function DemoBottomNav({
  ...
  return (
    <div className="fixed bottom-[max(1rem,env(safe-area-inset-bottom))] left-1/2 -translate-x-1/2 z-[9999] flex max-w-[calc(100vw-1rem)] items-center gap-1.5 overflow-x-auto bg-slate-900/95 backdrop-blur-xl border border-slate-700/80 rounded-2xl px-3 py-2 shadow-2xl shadow-black/50 mobile-scroll-panel">
      {/* Demo Badge */}
      ...
      {NAV_ITEMS.map(({ view, icon: Icon, label }) => (
        <button
          ...
          className={`min-h-11 flex flex-col items-center justify-center gap-0.5 px-3 py-1.5 rounded-xl transition-all text-xs font-bold ${
            currentView === view
      ...
      <button
        onClick={onTeacher}
        className="min-h-11 flex flex-col items-center justify-center gap-0.5 px-3 py-1.5 rounded-xl text-slate-400 hover:text-emerald-300 hover:bg-emerald-500/10 transition-all text-xs font-bold"
      >
      ...
      <button
        onClick={onExit}
        className="min-h-11 flex flex-col items-center justify-center gap-0.5 px-3 py-1.5 rounded-xl text-slate-500 hover:text-rose-400 hover:bg-rose-500/10 transition-all text-xs font-bold"
      >
